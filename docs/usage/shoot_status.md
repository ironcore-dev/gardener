...
If the webhook has rules for `DELETE` Pods, It could happen that, during hibernation kube-controller-manager is trying to cleanup pods, but if the pod backing the webhook is already gone, it cannot serve admission requests anymore and the deletion of other pods can fail. Even if the `kube-system` namespace is excluded, deletion of pods in other namespace can block hibernation. To prevent this, you should not configure webhooks with `DELETE` operation for pods, if you want to make use of the Shoot hibernation feature.
