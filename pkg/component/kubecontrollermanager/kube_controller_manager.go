...
func (k *kubeControllerManager) computeCommand(port int32) []string {
	...
	for api, enabled := range k.values.RuntimeConfig {
		...
		if controllerVersionRange, present := kubernetesutils.APIGroupControllerMap[api]; present {
			for controller, versionRange := range controllerVersionRange {
				contains, err := versionRange.Contains(k.values.TargetVersion.String())
				if err == nil {
					if contains {
	...
	// controllers := controllersToEnable.Difference(controllersToDisable)
	controllers := "--controllers=" + strings.Join(sets.List(controllersToEnable.Difference(controllersToDisable)), ",")
	if controllersToDisable.Len() > 0 {
		controllers += ",-" + strings.Join(sets.List(controllersToDisable), ",-")
	}

	command = append(command, controllers)
